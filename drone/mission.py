...
class Mission:
    def __init__(self, drone_controller, initial_yaw=0, map_file="map.json", target_groups=4):
        ...
        # Переменные для неблокирующего возврата и поворота
        self.return_phase = None
        self._return_cmd_sent = False
    ...
    def _go_to_point(self, x, y, z, yaw=None):
        print('_go_to_local_point')
        if yaw is None:
            yaw = self.current_yaw
        print(x, y, z)
        self.drone.drone.go_to_local_point(x, y, z, yaw=yaw)
        self.drone.drone.go_to_local_point(0, 8, 2, None)
    ...
    def update(self, markers_list):
        ...
        # ---- MEASURE_DIST ----
            ...
            if pair:
                ...
                if dist <= 0:
                    dist = 2.0
                    print(f"[Миссия] Предупреждение: dist <= 0, используем {dist}")
        ...
        # ---- NAVIGATE ----
            ...
            # Летим на gap * 1.5. Если не нашли, идём на возврат
            if dist_traveled > self.gap_distance * 1.5:
                print("[Миссия] Пройдено расстояние gap*1.5, пара не найдена. Возвращаемся.")
                self.drone.stop_with_inertia()
        ...
        # ---- RETURN_TO_LAST (Неблокирующий возврат и спиральный поиск) ----
            ...
            if self.return_phase is None:
                self.return_phase = 'GO_TO_ANCHOR'
                self._return_cmd_sent = False

            # Фаза 1: Возврат на последнюю сохранённую точку
            if self.return_phase == 'GO_TO_ANCHOR':
                # print(self._return_cmd_sent)
                if not self._return_cmd_sent:
                    # x, y, z = self.anchor_pos
                    # print(x, y, z)
                    self._go_to_point(0, 8, 2, yaw=np.radians(180))
                    self._return_cmd_sent = True
                
                if self.drone.drone.point_reached():
                    self.return_phase = 'ROTATE'
                    self._return_cmd_sent = False
                    self.rotation_attempts += 1
                    
                    if self.rotation_attempts > 3:
                        print("[Миссия] Все 4 направления проверены, меток нет. Завершаем.")
                        self.state = MissionState.COMPLETE
                        return 'done'
                    
                    # Поворачиваем курс на 90 градусов
                    self.current_yaw += np.radians(90)
                    print(f"[Миссия] Возврат выполнен. Поворот на 90° (попытка {self.rotation_attempts}). Новый курс: {np.degrees(self.current_yaw):.1f}°")
                    self._go_to_point(self.anchor_pos[0], self.anchor_pos[1], self.anchor_pos[2], yaw=self.current_yaw)
                    self.rotate_start_time = time.time()
                    self._return_cmd_sent = True

            # Фаза 2: Ожидание завершения поворота (по таймеру, т.к. point_reached может сработать сразу)
            elif self.return_phase == 'ROTATE':
                if time.time() - self.rotate_start_time > 2.0:
                    self.return_phase = 'FLY_NEW_DIR'
                    self._return_cmd_sent = False
                    
                    if self.gap_distance is None or self.gap_distance <= 0:
                        self.gap_distance = 2.0
                    
                    # Считаем новую целевую точку на расстоянии gap * 1.5
                    dist_to_fly = self.gap_distance * 1.5
                    target_x = self.anchor_pos[0] + dist_to_fly * np.sin(self.current_yaw)
                    target_y = self.anchor_pos[1] + dist_to_fly * np.cos(self.current_yaw)
            ...
            # Фаза 3: Полёт в новом направлении
            elif self.return_phase == 'FLY_NEW_DIR':
                if self.drone.drone.point_reached():
                    print("[Миссия] Достигли новой точки. Переход к поиску.")
                    self.anchor_pos = self.temp_target
                    self.state = MissionState.NAVIGATE
                    self.step_start_time = time.time()
                    self.last_move_time = time.time()
                    self.return_phase = None
