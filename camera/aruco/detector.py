...
class ArUcoDetector:
    def __init__(self):
        ...
        # Детектор для 5x5 (если нужно 6x6, замени DICT_5X5_50 на DICT_6X6_50)
        self.dict_5x5 = cv2.aruco.getPredefinedDictionary(cv2.aruco.DICT_5X5_50)
    ...
    def _compute_marker_info(self, corner, marker_id, marker_type):
        """Вычисляет центр и координаты для одного маркера."""
        c = corner[0]
        x_center = int(np.mean([p[0] for p in c]))
        y_center = int(np.mean([p[1] for p in c]))
        success, rvecs, tvecs = cv2.solvePnP(
            self.marker_points, corner,
    ...
    def process_frame(self, frame):
        ...
        if ids_5 is not None and len(ids_5) > 0:
            ...
            for corner, marker_id in zip(corners_5, ids_5.flatten()):
                ...
                markers.append(info)
                cv2.circle(frame, info['center'], 5, (0, 255, 0), -1)  # зелёный для 5x5
